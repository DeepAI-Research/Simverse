...
def setup_compositor_for_black_and_white(context):
    """
    ...
    composite = tree.nodes.new(type='CompositorNodeComposite')  # Ensure there's a composite node
    ...
    hue_sat.inputs['Saturation'].default_value = 0  # Reduce saturation to zero to get grayscale
    # increase contrast
    ...
    links.new(hue_sat.outputs['Image'], composite.inputs['Image'])  # Direct output to the composite node

def setup_compositor_for_cel_shading(context):
    """
    ...
    color_ramp_shadow = tree.nodes.new(type='CompositorNodeValToRGB')
    color_ramp_highlight = tree.nodes.new(type='CompositorNodeValToRGB')
    mix_rgb_shadow = tree.nodes.new(type='CompositorNodeMixRGB')
    mix_rgb_highlight = tree.nodes.new(type='CompositorNodeMixRGB')
    alpha_over = tree.nodes.new(type='CompositorNodeAlphaOver')  # Add an Alpha Over node
    ...
    mix_rgb_highlight.blend_type = 'SCREEN'  # Change to 'SCREEN' for better highlight blending
    ...
    links.new(mix_rgb_highlight.outputs['Image'], alpha_over.inputs[2])  # Plug the cel-shaded result into the foreground input of Alpha Over
    links.new(render_layers.outputs['Env'], alpha_over.inputs[1])  # Plug the environment pass into the background input of Alpha Over
    links.new(alpha_over.outputs['Image'], composite.inputs['Image'])  # Plug the Alpha Over output into the Composite node

def setup_compositor_for_depth(context):
    """
    ...
    links.new(normalize.outputs['Value'], composite.inputs['Image'])  # Direct output to the composite node
...
def enable_effect(context, effect_name):
    """
