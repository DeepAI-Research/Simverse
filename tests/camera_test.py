...
def test_reset_cameras():
    ...
    scene.objects = {
        ...
        "CameraOrientationPivotPitch": MagicMock(),
        "CameraFramingPivot": MagicMock(),
        "CameraAnimationPivot": MagicMock(),
    }
...
def test_sample_point_on_sphere():
    ...
    assert math.isclose(
        math.sqrt(point[0] ** 2 + point[1] ** 2 + point[2] ** 2), radius, abs_tol=1e-6
    ), "Point is not on the surface of the sphere"
