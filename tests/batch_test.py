...
def test_render_objects():
    ...
    with patch("subprocess.run") as mock_run, patch("os.makedirs"), patch(
        "os.path.exists", return_value=True
    ), patch("os.path.dirname", return_value="/fake/dir"), patch(
        "os.path.realpath", return_value="/fake/dir"
    ), patch.object(
        ...
        command = args[0][
            1
        ]  # This depends on how the command is structured in your actual function
...
if __name__ == "__main__":
    ...
    test_render_objects()
    test_render_objects_missing_blender()
