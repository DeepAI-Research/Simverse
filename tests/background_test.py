...
def test_set_background():
    ...
    with patch("background.get_background"), patch(
        "background.get_background_path", return_value="/fake/path/test_dataset/123.hdr"
    ), patch.object(bpy.data, "images", create=True) as mock_images, patch(
        "bpy.context"
    ):
...
def test_create_photosphere():
    ...
    with patch("bpy.ops.mesh.primitive_uv_sphere_add"), patch(
        "bpy.ops.object.shade_smooth"
    ), patch("bpy.ops.object.mode_set"), patch("bpy.ops.mesh.select_all"), patch(
        "bpy.ops.mesh.flip_normals"
    ), patch(
        "bpy.context", create=True
    ) as mock_context, patch(
        "background.create_photosphere_material"
    ) as mock_create_material:
...
def test_create_photosphere_material():
    ...
    with patch.object(
        bpy.data, "materials", create=True
    ) as mock_materials, patch.object(
        bpy.data.images, "load", return_value=MagicMock()
    ) as mock_load, patch(
        "background.get_background_path", return_value="/fake/path/test_dataset/123.hdr"
    ):
