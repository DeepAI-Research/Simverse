...
def test_read_json_file():
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(test_data))) as mock_file, \
         patch("json.load", return_value=test_data) as mock_json_load:
...
def test_generate_caption():
    ...
    combination = {
        'objects': [{'name': 'Object1', 'description': 'A cool object'}, {'name': 'Object2', 'description': 'Another cool object'}],
    ...
    with patch('random.choice', side_effect=lambda x: x[0]):  # Always choose the first element
...
def test_generate_combinations():
    ...
    with patch('combiner.read_json_file', return_value={'models': {}, 'backgrounds': {}}), \
         patch('random.choice', side_effect=lambda x: x[0]), \
         patch('random.choices', side_effect=lambda x, weights: [x[0]]), \
         patch('random.randint', return_value=1):  # Return 1 for number of objects
